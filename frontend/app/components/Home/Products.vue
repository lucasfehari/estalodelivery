      ...
      >
        <div class="relative mb-3">
          ...
          <div class="absolute bottom-2 left-2 bg-white/95 backdrop-blur px-2 py-1 rounded-lg flex items-center gap-1 shadow-sm">
            ...
            <span class="text-[10px] font-bold text-slate-700 uppercase">{{ product.deliveryTime }}</span>
          </div>
        </div>
        <div class="px-1 pb-1">
          <h4 class="font-bold text-sm text-slate-800 line-clamp-1 mb-1 leading-none">{{ product.name }}</h4>
          <div class="flex items-center gap-2 mb-1">
            <span class="text-green-600 font-extrabold text-base">{{ product.price }}</span>
            <span class="text-slate-400 text-[10px] line-through">{{ product.originalPrice }}</span>
          </div>
        </div>
      </div>
    </div>
